...
#DEFAULT_RESOURCE_TYPE = 'AWS::RDS::DBProxy'
DEFAULT_RESOURCE_TYPE = 'AWS::RoboMaker::Robot'
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    try:
        rds_proxy_client = get_client("rds", event)
        rds_proxies = rds_proxy_client.describe_db_proxies()
        while True:
            for rds_proxy in rds_proxies['DBProxies']:
                proxy_name = rds_proxy['DBProxyName']
                tls_enabled = rds_proxy['RequireTLS']
                logger.info(f'AWS RDS Proxy Name: {proxy_name}')
                logging.info(f'TLS enaled status is {tls_enabled} for {proxy_name}')

                if tls_enabled:
                    logging.info(f'RDS Proxy {proxy_name} is COMPLIANT, TLS Encryption in Transit is enabled')
                    evaluations.append(build_evaluation(proxy_name, 'COMPLIANT', event))
                else:
                    violation_report = f'RDS Proxy {proxy_name} is Non-Compliant, TLS Encryption in Transit is disabled'
                    logger.info(violation_report)
                    evaluations.append(build_evaluation(proxy_name, 'NON_COMPLIANT', event, annotation=violation_report))

            if rds_proxies.get('Marker', "") == "":
                break
            else:
                rds_proxies = rds_proxy_client.describe_db_proxies(Marker=rds_proxies['Marker'])

    except Exception as error:
        logging.error(f"Failed to check TLS Encryption in Transit for AWS RDS Proxy in account number: {account_id}. Error details: {error}")

    logger.info(f"[config evaluations] {json.dumps(evaluations)}")
    return evaluations
...
def lambda_handler(event, context):
    ...
    # Timeout to prevent Config throttling
    rand = random.randint(1, 60)
    print(f'Timeout to prevent AWS Config throttling: {rand} secs')
    time.sleep(rand)
